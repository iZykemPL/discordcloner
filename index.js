...
console.log(" ");
console.log(" ");
let token = await question(('- daj twuj token:'.cyan));
let copy = await question(('- id serwa kture chcesz skopiowac:'.cyan));
let paste = await question(('- id serwa na ktorym chcesz to samo:'.cyan));
...
client.on("ready", async() => {
  ...
  let region = allowedRegions.includes(guild2.region) ? guild2.region : 'eu-central';
	...
	let guildname = guild1.name + " - Hackerz Cloner";
